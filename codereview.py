...
class CodeReviewer:
    ...
    def generate_review(self, file_path: Path) -> str:
        ...
        system_prompt = (
            ...
            "Output the COMPLETE original code, but insert review comments where you find issues.\n"
            "Issues to look for: Logic bugs, Security vulnerabilities, Performance bottlenecks, and Bad practices.\n"
            f"Use the comment prefix '{comment_prefix}' for your comments.\n"
            "Insert the comment immediately BEFORE the line it refers to.\n"
            "Do not remove any original code.\n"
            "Do not wrap the output in Markdown code blocks (like ```python ... ```). Just output the raw code."
        )
