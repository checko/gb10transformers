#!/usr/bin/env python3
"""
AI Code Review Client (Ollama Version)
Sends code and dynamic prompts to a remote Ollama server.

Usage:
    python review_client_ollama.py <file_or_folder>

Environment Variables:
    OLLAMA_HOST - Ollama server host (default: 192.168.145.70)
...
# ============================================================================
OLLAMA_HOST = os.getenv("OLLAMA_HOST", "192.168.145.70")
OLLAMA_PORT = int(os.getenv("OLLAMA_PORT", "11434"))
OLLAMA_MODEL = os.getenv("OLLAMA_MODEL", "qwen3-coder:30b")

# Ollama API endpoint - using the chat completion API
...
def send_ollama_request(code: str, prompt: str, timeout: int) -> str:
    """
    Send request to Ollama server using the /api/chat endpoint.
    
    The Ollama chat API expects:
    {
    ...
    try:
        ...
        payload = json.dumps({
            ...
            "options": {
                "temperature": 0.2,
                "num_predict": 4096  # max_new_tokens equivalent
            }
        })
        
        print(f"    [Ollama] Sending to {OLLAMA_HOST}:{OLLAMA_PORT} (model: {OLLAMA_MODEL})...")
        
        conn.request('POST', OLLAMA_API_PATH, payload, headers)
        response = conn.getresponse()
        
        if response.status != 200:
            error_body = response.read().decode()
            print(f"[ERROR] Ollama returned {response.status}: {response.reason}")
        ...
        if 'eval_count' in data:
            ...
            if eval_duration > 0:
                print(f"    [Ollama] Generated {eval_count} tokens in {eval_duration:.1f}s ({eval_count/eval_duration:.1f} tok/s)")
        
        return clean_response(content)
        
    except ConnectionRefusedError:
        print(f"[ERROR] Could not connect to Ollama server at {OLLAMA_HOST}:{OLLAMA_PORT}")
        print("[ERROR] Make sure Ollama is running and accessible.")
        sys.exit(1)
...
def review_file(file_path: Path):
    ...
    print(f"  Using Ollama server: {OLLAMA_HOST}:{OLLAMA_PORT} / {OLLAMA_MODEL}")
    ...
    while chunk_start <= line_count:
        ...
        chunk_diff = send_ollama_request(content, current_prompt, dynamic_timeout)
    ...
    if full_diff.strip():
        ...
        # Use .ollama.diff suffix to differentiate from local model output
        output_path = file_path.parent / (file_path.name + ".ollama.diff")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python review_client_ollama.py <file_or_folder>")
        print()
        print("Environment Variables:")
        print(f"  OLLAMA_HOST  - Ollama server host (current: {OLLAMA_HOST})")
        print(f"  OLLAMA_PORT  - Ollama server port (current: {OLLAMA_PORT})")
